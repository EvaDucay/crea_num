...
screen scoreUI: 
    frame:
        image "Backgrounds/tache-sang.png" xpos -60 ypos -80 zoom 0.4
        xsize 90
        ysize 70
    timer 0.1 action If(round(countdownTime) > 0, true = SetVariable("countdownTime", countdownTime - 0.1), false = Show("gameOverLose")) repeat If(round(countdownTime) > 0, true = True, false = NullAction())# every 1 second do ...
    text "{:.0f}".format(countdownTime) align(0.03, 0.015) color "#000000"
    text "{:.0f}".format(zombi_kill) align(0.03, 0.06) color "#000000"
    frame:
        # background "#f61307f5"
